...
//! with flexible configuration for custom headers, endpoints, and authentication.
...
pub struct CustomConfig {
    ...
    pub api_format: Option<String>, // "openai" or custom
}
...
impl Default for CustomConfig {
    fn default() -> Self {
        Self {
            ...
            embedding_endpoint: Some("/embeddings".to_string()),
...
impl LLMProvider for CustomProvider {
    ...
    async fn chat_completion(
        ...
        let usage = response_data["usage"].as_object().map(|usage| Usage {
            prompt_tokens: usage["prompt_tokens"]
                .as_u64()
                .unwrap_or(total_tokens as u64) as u32,
        ...
        Ok(ChatCompletionResponse {
            id: response_data["id"]
                .as_str()
                .unwrap_or(&format!("custom_{}", chrono::Utc::now().timestamp()))
                .to_string(),
                    ...
                    if line.starts_with("data: ") {
                        ...
                        if let Ok(json) = serde_json::from_str::<serde_json::Value>(data) {
                            let chunk = ChatCompletionChunk {
                                id: json["id"].as_str().unwrap_or("custom_stream").to_string(),
    ...
    async fn embedding(&self, request: EmbeddingRequest) -> Result<EmbeddingResponse, Self::Error> {
        ...
        let embeddings: Vec<EmbeddingData> = serde_json::from_value(response_data["data"].clone())
            .map_err(|e| ProviderError::Internal(format!("Failed to parse embeddings: {}", e)))?;
    ...
    async fn list_models(&self) -> Result<Vec<Model>, Self::Error> {
        let endpoint = self.config.models_endpoint.as_deref().unwrap_or("/models");
        let mut req_builder = self.client.get(&format!("{}{}", self.base_url, endpoint));
    ...
    async fn get_cost_info(&self, _model: &str) -> Result<CostInfo, Self::Error> {
        // Custom providers may have custom pricing
        Ok(CostInfo {
